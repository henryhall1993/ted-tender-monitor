...
INPUT_CSV  = "results/scotland_medical_devices.csv"
OUTPUT_CSV = "results/scotland_medical_devices_parsed.csv"
...
Below is a public tender notice from Scotland.
...
def parse_with_claude(title, buyer, description):
    try:
...
def main():
    if not os.path.exists(INPUT_CSV):
        print(f"Input file {INPUT_CSV} not found — skipping Scotland parsing.")
        return

    with open(INPUT_CSV, newline='', encoding='utf-8') as f:
        rows = list(csv.DictReader(f))

    print(f"Found {len(rows)} Scotland tenders. Processing all...\n")
    ...
    for i, row in enumerate(rows):
        ...
        print(f"[{i+1}/{len(rows)}] {notice_id} — {title}")

        if not description:
            print("  No description — skipping Claude call")
            claude_response = ""
        else:
            print("  Parsing with Claude...")
            claude_response = parse_with_claude(title, buyer, description)

        result = {
            "notice_id":               notice_id,
            "original_title":          title,
            "buyer_name":              buyer,
            "country":                 row.get("country", "SCT"),
    ...
    print(f"\nDone — {len(results)} Scotland tenders parsed and saved to {OUTPUT_CSV}")
